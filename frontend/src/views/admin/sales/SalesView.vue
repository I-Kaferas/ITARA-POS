...
import { emptyListFilters, periodRange, type ListFilters } from '../../../utils/listFilters'
...
const paymentOptions = computed(() => [
  ...
  { value: 'unpaid', label: t('pointOfSale.orders.payment.unpaid') },
  { value: 'on_credit', label: t('pointOfSale.orders.payment.on_credit') },
])

async function load() {
  if (!storeId.value) return
  const range = periodRange(filters.value.period, filters.value.from, filters.value.to)
  await store.loadSales(storeId.value, {
    q: filters.value.search || undefined,
    status: filters.value.status || undefined,
    payment_status: filters.value.payment_method || undefined,
    ...range,
  })
}
...
function paymentStatusLabel(status: string): string {
  const map: Record<string, string> = {
    paid: t('pointOfSale.orders.payment.paid'),
    partial: t('pointOfSale.orders.payment.partial'),
    unpaid: t('pointOfSale.orders.payment.unpaid'),
    credit: t('pointOfSale.orders.payment.credit'),
    on_credit: t('pointOfSale.orders.payment.on_credit'),
  }
...
<template>
  <SalesLayout>
    ...
    <div v-else class="space-y-3">
      <ModuleFilters
        v-model="filters"
        :search-placeholder="t('pointOfSale.orders.filters.searchPlaceholder')"
