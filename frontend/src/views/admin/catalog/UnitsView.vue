    ...
    >
      <form class="space-y-3" @submit.prevent="save">
        <div class="grid grid-cols-2 gap-3">
          ...
          <div><label class="mb-1 block text-sm font-medium">{{ t('catalog.symbol') }}</label><input v-model="form.symbol" class="field" /></div>
        </div>
        <div><label class="mb-1 block text-sm font-medium">{{ t('org.name') }}</label><input v-model="form.name" required class="field" /></div>
        <label class="flex items-center gap-2 text-sm"><input v-model="form.is_fractional" type="checkbox" class="rounded" />{{ t('catalog.fractional') }}</label>
